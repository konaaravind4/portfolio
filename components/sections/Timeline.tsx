...
import { BookOpen, Briefcase, Award, GraduationCap, Flame } from "lucide-react";
    ...
    return (
        <section id="timeline" className="py-28 relative" style={{ background: "transparent" }}>
            ...
            <div className="absolute inset-0 pointer-events-none"
                style={{ background: "radial-gradient(ellipse at 30% 50%, rgba(139,0,0,0.04) 0%, transparent 60%)" }} />
            ...
            <div className="relative max-w-4xl mx-auto px-4 sm:px-6">
                <motion.div
                    ...
                    <h2 className="section-heading mb-4 text-glow-gold">Education & Experience</h2>
                    <div className="section-divider max-w-xs mx-auto" />
                    <p className="text-sm max-w-lg mx-auto" style={{ color: "var(--text-muted)" }}>
                        From academic foundations in AI to real-world internships and professional certifications.
                    </p>
                </motion.div>
                ...
                <div className="relative">
                    ...
                    <div className="absolute left-6 top-0 bottom-0 w-px"
                        style={{ background: "linear-gradient(to bottom, rgba(201,168,76,0.5), rgba(139,0,0,0.3), rgba(201,168,76,0.1), transparent)" }} />
                        ...
                        {events.map((event, i) => {
                            ...
                            return (
                                <motion.div key={i}
                                    ...
                                    transition={{ type: "spring", stiffness: 55, damping: 18, delay: i * 0.15 }}
                                    className="relative flex gap-6"
                                    // Anti-gravity subtle float
                                    animate={{ y: isHovered ? -4 : 0 }}
                                >
                                    {/* Icon bubble — floating */}
                                    <motion.div
                                        className="relative z-10 flex-shrink-0 w-12 h-12 border flex items-center justify-center"
                                        style={{
                                        ...
                                        animate={{ y: [0, -6, 0], boxShadow: isHovered
                                            ? [`0 0 12px ${event.color}40`, `0 0 25px ${event.color}60`, `0 0 12px ${event.color}40`]
                                            : `0 0 0px transparent`
                                        }}
                                    ...
                                    {/* Content card */}
                                    <motion.div
                                        className="cursor-target flex-1 p-5 corner-ornament"
                                        style={{
                                            background: isHovered ? "rgba(22, 18, 10, 0.9)" : "rgba(16, 14, 10, 0.7)",
                                            border: `1px solid ${isHovered ? event.color + '55' : 'rgba(201,168,76,0.12)'}`,
                                            backdropFilter: "blur(12px)",
                                            clipPath: "polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))",
                                            transition: "all 0.35s ease",
                                            boxShadow: isHovered ? `0 4px 30px rgba(0,0,0,0.5), 0 0 20px ${event.color}18` : "none"
                                        }}
                                        onMouseEnter={() => setHoveredIdx(i)}
                                        onMouseLeave={() => setHoveredIdx(null)}>
                                        <div className="flex flex-wrap items-center justify-between gap-2 mb-1">
                                            <span className="text-xs font-semibold uppercase tracking-widest"
                                                style={{ color: event.color, fontFamily: "'JetBrains Mono', monospace" }}>
                                                {event.type}
                                            </span>
