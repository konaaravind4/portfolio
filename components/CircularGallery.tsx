...
class Title {
    ...
    Texture: any; Plane: any; Program: any; Mesh: any;
    constructor({ gl, plane, renderer, text, textColor = "#545050", font = "30px sans-serif", Texture, Plane, Program, Mesh }: any) {
        autoBind(this);
        this.gl = gl; this.plane = plane; this.renderer = renderer;
        this.text = text; this.textColor = textColor; this.font = font;
        this.Texture = Texture; this.Plane = Plane; this.Program = Program; this.Mesh = Mesh;
        this.createMesh();
    }
    createMesh() {
        const { texture, width, height } = createTextTexture(this.gl, this.text, this.font, this.textColor, this.Texture);
        const geometry = new this.Plane(this.gl);
        const program = new this.Program(this.gl, {
        ...
        this.mesh = new this.Mesh(this.gl, { geometry, program });
...
class Media {
    ...
    title: any; speed: number; x: number; width: number; widthTotal: number; padding: number;
    isBefore: boolean; isAfter: boolean;
    ogl: any;

    constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font, ogl }: any) {
        this.extra = 0; this.geometry = geometry; this.gl = gl; this.image = image;
        this.index = index; this.length = length; this.renderer = renderer; this.scene = scene;
        this.screen = screen; this.text = text; this.viewport = viewport; this.bend = bend;
        this.textColor = textColor; this.borderRadius = borderRadius; this.font = font; this.ogl = ogl;
        this.speed = 0; this.x = 0; this.width = 0; this.widthTotal = 0; this.padding = 0;
        this.isBefore = false; this.isAfter = false;
...
class App {
    ...
    planeGeometry: any; medias: any[]; mediasImages: any[]; raf: number;
    boundOnResize: any; boundOnWheel: any; boundOnTouchDown: any;
    boundOnTouchMove: any; boundOnTouchUp: any; isDown: boolean; start: number;
    ogl: any;

    constructor(container: HTMLElement, { items, bend, textColor = "#ffffff", borderRadius = 0, font = "bold 30px Figtree", scrollSpeed = 2, scrollEase = 0.05, ogl }: any) {
        this.container = container; this.scrollSpeed = scrollSpeed; this.ogl = ogl;
        this.scroll = { ease: scrollEase, current: 0, target: 0, last: 0 };
        this.onCheckDebounce = debounce(this.onCheck.bind(this), 200);
        this.medias = []; this.mediasImages = []; this.raf = 0; this.isDown = false; this.start = 0;
    ...
    createScene() { const { Transform } = this.ogl; this.scene = new Transform(); }
    createGeometry() {
    ...
    createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
        ...
        this.medias = this.mediasImages.map((data, index) =>
            new Media({
                ...
                bend, textColor, borderRadius, font, ogl: this.ogl,
            })
        );
    }
...
export default function CircularGallery({
    ...
    useEffect(() => {
        ...
        import("ogl").then((ogl) => {
            app = new App(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, ogl });
        });
